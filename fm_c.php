...
// CREATE: Assign Equipment to a Student (Without modifying existing code)
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["create_inventory"])) {
    $user_id = $_POST["user_id"];
    $inventory_id = $_POST["inventory_id"];
    $return_date = $_POST["return_date"];

    $stmt = $connection->prepare("INSERT INTO student_inventory (user_id, inventory_id, borrowed_date, return_date, status) VALUES (?, ?, NOW(), ?, 'Assigned')");
    $stmt->bind_param("iis", $user_id, $inventory_id, $return_date);
