...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = $connection->prepare("SELECT email, password FROM users WHERE user_id = ?");
    $query->bind_param("i", $user_id);
    $query->execute();
    $query->bind_result($registered_email, $old_password);
    $query->fetch();
    $query->close();
    ...
    if ($encrypted_password === $old_password) {
        die("New passwords cannot be the same as the old one. Please try again.");
    }
